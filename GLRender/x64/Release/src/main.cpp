...
GLchar* LoadShader(const std::string& shaderFile) {
	...
	memcpy(shader, file_contents.c_str(), file_contents.length() + 1);
...
int main(int argc, char** argv) {
	...
	shadowVertexShaderCompileLog.open("shdowVertexShader.txt", std::ios::out);
	...
	vertexFragShaderCompileLog.open("shdowFragShader.txt", std::ios::out);
	...
	matShadowProj = glm::perspective(glm::radians(90.f), aspect, 0.1f, 100.f);
	...
	for (GLuint i = 0; i < 6; i++) {
		glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_R32F, SCREEN_WIDTH, SCREEN_HEIGHT, 0, GL_RED, GL_FLOAT, NULL);
	}
	...
	*/
	...
	Model companion_cube("crate2.obj", uniModel, shaderProgram, shadowShaderProgram);
	companion_cube.translate(glm::vec3(3, 0.5, 2));
	
	Model quake_crate("crate.obj", uniModel, shaderProgram, shadowShaderProgram);
	quake_crate.translate(glm::vec3(-3, 0.5, -2));

	Model room("room.obj", uniModel, shaderProgram, shadowShaderProgram);
	...
	while (running) {
		...
		if (ticksSinceRender >= renderInterval) {
			...
			for (GLuint i = 0; i < 6; i++) {
				...
				glm::mat4 shadowCamera = glm::lookAt(lightPos, gCameraDirections[i].Target, gCameraDirections[i].Up);
			...
			glBindTexture(GL_TEXTURE_CUBE_MAP, shadowMap);
			glUniform1i(glGetUniformLocation(shaderProgram, "shadowMap"), shadowMap);
